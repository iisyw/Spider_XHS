...
def check_or_create_download_record(csv_path, user_id):
    ...
    if not os.path.exists(csv_file):
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['note_id', 'user_id', 'note_type', 'title', 'download_time', 'is_complete'])
        ...
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if row and row[0] == note_id:
                    # 更新现有记录
                    row[4] = download_time
                    row[5] = str(is_complete)
                    updated = True
                rows.append(row)
    
    # 如果没有现有记录，添加新记录
    if not updated:
        rows.append([note_id, user_id, note_type, title, download_time, str(is_complete)])
